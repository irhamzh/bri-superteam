...
import { deleteCatering } from '../../../modules/catering/actions'
...
function Catering(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteCatering } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteCatering(id, doRefresh)
        }
      })
      .catch((err) => {
...
Catering.propTypes = {
  ...
  deleteCatering: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteCatering: (id, refresh) => dispatch(deleteCatering(id, refresh)),
})
