...
import { deleteCheckpoint } from '../../../modules/master/checkpoint/actions'
...
function Checkpoint(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteCheckpoint } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteCheckpoint(id, doRefresh)
        }
      })
      .catch((err) => {
...
Checkpoint.propTypes = {
  ...
  deleteCheckpoint: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteCheckpoint: (id, refresh) => dispatch(deleteCheckpoint(id, refresh)),
})
