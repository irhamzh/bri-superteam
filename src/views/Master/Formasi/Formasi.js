...
import { deleteFormasi } from '../../../modules/master/formasi/actions'
...
function Formasi(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteFormasi } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteFormasi(id, doRefresh)
        }
      })
      .catch((err) => {
...
Formasi.propTypes = {
  ...
  deleteFormasi: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteFormasi: (id, refresh) => dispatch(deleteFormasi(id, refresh)),
})
