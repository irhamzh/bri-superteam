...
import { deleteGedung } from '../../../modules/gedung/actions'
...
function Gedung(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteGedung } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteGedung(id, doRefresh)
        }
      })
      .catch((err) => {
...
Gedung.propTypes = {
  ...
  deleteGedung: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteGedung: (id, refresh) => dispatch(deleteGedung(id, refresh)),
})
