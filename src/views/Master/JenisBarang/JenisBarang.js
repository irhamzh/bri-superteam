...
import { deleteJenisBarang } from '../../../modules/jenisBarang/actions'
...
function JenisBarang(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteJenisBarang } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteJenisBarang(id, doRefresh)
        }
      })
      .catch((err) => {
...
const mapDispatchToProps = (dispatch) => ({
  deleteJenisBarang: (id, refresh) => dispatch(deleteJenisBarang(id, refresh)),
})
...
export default connect(
  ...
  withTableFetchQuery({
    API: (p) => Service.getJenisBarang(p),
