...
import { deleteJenisPC } from '../../../modules/jenisPC/actions'
...
function JenisPC(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteJenisPC } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteJenisPC(id, doRefresh)
        }
      })
      .catch((err) => {
...
JenisPC.propTypes = {
  ...
  deleteJenisBarang: PropTypes.func.isRequired,
...
const mapStateToProps = (state) => ({
  ...
  isLoading: state.jenisPC.isLoading,
  message: state.jenisPC.message,
})

const mapDispatchToProps = (dispatch) => ({
  deleteJenisPC: (id, refresh) => dispatch(deleteJenisPC(id, refresh)),
})
