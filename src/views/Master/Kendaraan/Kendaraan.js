...
import { deleteKendaraan } from '../../../modules/master/kendaraan/actions'
...
function Kendaraan(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteKendaraan } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteKendaraan(id, doRefresh)
        }
      })
      .catch((err) => {
...
Kendaraan.propTypes = {
  ...
  deleteKendaraan: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteKendaraan: (id, refresh) => dispatch(deleteKendaraan(id, refresh)),
})
