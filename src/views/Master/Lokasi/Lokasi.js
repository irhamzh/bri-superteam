...
import { deleteLokasi } from '../../../modules/lokasi/actions'
...
function Lokasi(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteLokasi } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteLokasi(id, doRefresh)
        }
      })
      .catch((err) => {
...
Lokasi.propTypes = {
  ...
  deleteLokasi: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteLokasi: (id, refresh) => dispatch(deleteLokasi(id, refresh)),
})
