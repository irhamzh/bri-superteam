...
import { deletePajak } from '../../../modules/master/pajak/actions'
...
function Pajak(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deletePajak } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deletePajak(id, doRefresh)
        }
      })
      .catch((err) => {
...
Pajak.propTypes = {
  ...
  deletePajak: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deletePajak: (id, refresh) => dispatch(deletePajak(id, refresh)),
})
