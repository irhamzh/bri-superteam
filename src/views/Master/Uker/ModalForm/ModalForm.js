...
import { createUker, updateUker } from '../../../../modules/master/uker/actions'
...
function ModalForm(props) {
  ...
  function handleSaveChanges(values) {
    const { id } = values
    const { createUker, updateUker } = props
    if (!invalidValues.includes(id)) {
      updateUker(values, id, doRefresh)
    } else {
      createUker(values, doRefresh)
    }
  }
    ...
    <>
      ...
      >
        ...
        >
          {({ isSubmitting }) => (
            <Form>
              <ModalHeader toggle={modalForm.hide}>Form Uker</ModalHeader>
...
ModalForm.propTypes = {
  ...
  createUker: PropTypes.func.isRequired,
  updateUker: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  createUker: (formData, refresh) => dispatch(createUker(formData, refresh)),
  updateUker: (formData, id, refresh) => dispatch(updateUker(formData, id, refresh)),
})
