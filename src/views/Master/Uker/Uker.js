...
import { deleteUker } from '../../../modules/master/uker/actions'
...
function Uker(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteUker } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteUker(id, doRefresh)
        }
      })
      .catch((err) => {
...
Uker.propTypes = {
  ...
  deleteUker: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  deleteUker: (id, refresh) => dispatch(deleteUker(id, refresh)),
})
