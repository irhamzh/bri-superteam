...
import { deleteUnitPompa } from '../../../modules/unitPompa/actions'
...
function UnitPompa(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteUnitPompa } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteUnitPompa(id, doRefresh)
        }
      })
      .catch((err) => {
...
UnitPompa.propTypes = {
  ...
  deleteUnitPompa: PropTypes.func.isRequired,
...
const mapStateToProps = (state) => ({
  ...
  isLoading: state.unitPompa.isLoading,
  message: state.unitPompa.message,
})

const mapDispatchToProps = (dispatch) => ({
  deleteUnitPompa: (id, refresh) => dispatch(deleteUnitPompa(id, refresh)),
})
