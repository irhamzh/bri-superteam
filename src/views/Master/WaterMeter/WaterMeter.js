...
import { deleteWaterMeter } from '../../../modules/waterMeter/actions'
...
function WaterMeter(props) {
  ...
  function handleDelete(e, state) {
    ...
    const { id } = state
    const { deleteWaterMeter } = props

    AlertMessage.warning()
      .then((result) => {
        if (result.value) {
          deleteWaterMeter(id, doRefresh)
        }
      })
      .catch((err) => {
...
const mapStateToProps = (state) => ({
  ...
  isLoading: state.waterMeter.isLoading,
  message: state.waterMeter.message,
})

const mapDispatchToProps = (dispatch) => ({
  deleteWaterMeter: (id, refresh) => dispatch(deleteWaterMeter(id, refresh)),
})
